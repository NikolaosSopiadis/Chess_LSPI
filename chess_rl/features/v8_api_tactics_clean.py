...
# Adjust this import/name if your actual v4 feature class has a different name.
...
class V8ApiTacticsCleanFeatures:
    """
    v8_api_tactics_clean = v4_slim + selected legal-move tactical/API features.
    ...
    Added:
      37 legal_checking_moves_diff
      38 safe_checking_moves_diff
      39 mate_in_one_threat_diff
      40 safe_capture_value_diff
      41 unsafe_capture_liability_diff
      42 queen_tempo_threat_diff
      43 queen_tempo_threat_by_minor_or_pawn_diff

    Intentionally removed from v7:
      - broad attack-map features
      - queen-pressure block
      - loose/overload pressure block
      - tempo_liability_diff
    """

    spec = _FeatureSpec(version="v8_api_tactics_clean", dim=44)
    ...
    def phi_afterstate(self, board: Board) -> np.ndarray:
        ...
        if base.shape[0] != 37:
            raise ValueError(
                f"V8ApiTacticsCleanFeatures expected v4_slim dim 37, got {base.shape[0]}"
            )
        ...
        if out.shape[0] != self.spec.dim:
            raise ValueError(
                f"v8_api_tactics_clean produced dim {out.shape[0]}, expected {self.spec.dim}"
            )
            ...
            for move in moves:
                ...
                if move.flags & F_CAPTURE:
                    ...
                    if self._landing_square_safe_after(board, move, mover_white=white):
                        ...
                        # Crude SEE-like penalty.
                        # Bad: queen takes pawn and can be recaptured.
                        # Less bad: knight takes queen and can be recaptured.
    ...
    def _queen_tempo_threat_diff(
        ...
        *,
        ...
        """
        Positive means White has more legal moves attacking Black's queen.
        Negative means Black has more legal moves attacking White's queen.
        """
    ...
    def _landing_square_safe_after(
        ...
        *,
        ...
        This is simple, not a full SEE.
        """
        with board.temporary_move(move):
            return not board.is_square_attacked(
                move.dst_square,
                by_white=not mover_white,
            )
